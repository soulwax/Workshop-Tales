// File: src/de/soulwax/wstales/level/tile/CloudTile.java
...
public class CloudTile extends Tile {
	...
	/*
	 * public boolean interact(Level level, int xt, int yt, Player player, Item
	 * item, int attackDir) { if (item instanceof ToolItem) { ToolItem tool =
	 * (ToolItem) item; if (tool.type == ToolType.pickaxe) { if (player.payStamina(4
	 * - tool.level)) { hurt(level, xt, yt, random.nextInt(10) + (tool.level) * 5 +
