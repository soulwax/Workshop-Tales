// File: src/de/soulwax/wstales/level/tile/WheatTile.java
...
public class WheatTile extends Tile {
	...
	private void harvest(Level level, int x, int y) {
		...
		for (int i = 0; i < count; i++) {
			level.add(new ItemEntity(new ResourceItem(Resource.seeds), x * 16 + random.nextInt(10) + 3,
		...
		for (int i = 0; i < count; i++) {
			level.add(new ItemEntity(new ResourceItem(Resource.wheat), x * 16 + random.nextInt(10) + 3,
					y * 16 + random.nextInt(10) + 3));
		}
